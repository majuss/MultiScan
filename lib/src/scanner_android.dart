...
class LanScanner extends LanScannerCore {
  LanScanner({
    ...
    super.preferredInterfaceNames = ScannerDefaults.preferredInterfaceNames,
          ...
          reverseDnsTimeoutMs:
              reverseDnsTimeoutMs ?? ScannerDefaults.androidReverseDnsTimeoutMs,
        );
}

class AndroidScannerPlatform implements ScannerPlatform {
  @override
  ...
  List<InterfaceInfo> filterInterfaces(List<InterfaceInfo> list,
      {String? wifiIp}) {
    final wifiOnly = <InterfaceInfo>[];
    if (wifiIp != null) {
      wifiOnly.addAll(
          list.where((entry) => entry.address.address == wifiIp));
    }
    if (wifiOnly.isEmpty) {
      wifiOnly.addAll(list.where((entry) {
        final name = entry.name.toLowerCase();
        return name.contains('wlan') || name.contains('wifi');
      }));
    }
    return wifiOnly.isNotEmpty ? wifiOnly : list;
  }
